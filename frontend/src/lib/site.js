...
export const SITE = {
    ...
    // Place your logo file at frontend/public/logo.png (any image format works —
    // just update the extension below to match your actual file).
    logoUrl: "/logo.png",
    social: {
...
// Returns candidate URLs for a generated/uploaded image, tried in order by
// LuxImg (png, then jpg, then jpeg) before falling back to a stock photo.
export function imgCandidates(name) {
    return ["png", "jpg", "jpeg"].map((ext) => `${API}/static/img/generated/${name}.${ext}`);
}

// Back-compat helper — returns the .png URL directly.
